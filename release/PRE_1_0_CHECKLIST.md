...
- [x] Dashboard panels created/imported. (`release/runtime-validation/dashboard/DASHBOARD_PANELS_EVIDENCE_2026-09-23.md` — all 8 panels from `implementations/wazuh/dashboard/DASHBOARD_SPEC.yml` generated via `implementations/wazuh/dashboard/generate_dashboard_ndjson.py`, imported into the real Wazuh Dashboard, and every panel's aggregation confirmed to execute against the real index mappings without error. **Visual browser rendering:** not confirmed by the automated pass — blocked by lab-host RAM headroom for a headless browser and by the coordinating session's own sandbox lacking root/package-install access. Project owner decision, 2026-09-24: verify directly by opening the dashboard in their own browser rather than installing browser tooling on the shared lab host. See `implementations/wazuh/DEPLOYMENT.md` section 5 for the steps followed. **Confirmed by project owner, 2026-09-24: opened the dashboard in a real browser and it renders correctly.**)
...
- [x] Role-based access model documented. **v3 live-tested successfully end to end, 2026-09-24.** (`release/runtime-validation/dashboard/RBAC_EVIDENCE_2026-09-24.md` — v1 found `pdp_dashboard_editor` could not access dashboard saved objects at all; v2 (fixing `cluster_composite_ops` and widening `.kibana*` actions, per OpenSearch Security's reference `kibana_user` role) still failed live, including read access for both roles. Root cause found via OpenSearch Security `DEBUG` logging (a logging-verbosity change, reverted after diagnosis, not a permission change): `WARN PrivilegesInterceptorImpl: Tenant global_tenant is not allowed for user <user>` — the security plugin's Kibana-multitenancy interceptor requires an explicit `tenant_permissions` grant for `global_tenant` regardless of the Dashboards-side `multitenancy.enabled` flag, which neither v1 nor v2 declared. v3 adds `tenant_permissions` (`kibana_all_read`/`kibana_all_write`) to both roles. All 8 test scenarios in `implementations/wazuh/dashboard/rbac/README.md` passed: viewer opens the dashboard and cannot save changes; editor opens the dashboard and can create/save a visualization; neither can write `pdp-*` data. Test users/index/visualization cleaned up afterward; the roles themselves were deliberately left live as the actual deliverable.)
