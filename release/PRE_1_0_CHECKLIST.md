...
- [ ] Role-based access model documented. **Live-tested 2026-09-24, partially correct.** (`release/runtime-validation/dashboard/RBAC_EVIDENCE_2026-09-24.md` — `pdp_dashboard_viewer` (read-only on `pdp-*` data) is **confirmed correct**: 3/3 index-level tests passed. `pdp_dashboard_editor` is **not yet correct**: its `crud` grant on `.kibana`/`.kibana_1` does not grant working access to the Dashboards saved-objects application layer — neither read nor write succeeded via the real `/api/saved_objects/...` path, despite the raw index permission being present (confirmed by a successful write to the concrete `.kibana_1` index outside the Dashboards API). Root cause not yet identified; likely needs an additional Dashboards-specific cluster permission beyond index CRUD. All test users/config changes were rolled back and verified removed from the live indexer — do not re-apply until the editor gap is resolved.)
