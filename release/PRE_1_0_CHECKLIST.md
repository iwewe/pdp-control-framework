...
- [ ] Role-based access model documented. **v1 live-tested 2026-09-24 (viewer correct, editor gap found); root cause researched same day; v2 fix designed but not yet live-tested.** (`release/runtime-validation/dashboard/RBAC_EVIDENCE_2026-09-24.md` — v1's `pdp_dashboard_viewer` was confirmed correct for `pdp-*` data protection (3/3 index-level tests passed), but `pdp_dashboard_editor` could not read or write dashboard saved objects via the real `/api/saved_objects/...` path. Root-cause research compared v1 against OpenSearch Security's own reference `kibana_user` role and found two likely gaps: `cluster_composite_ops_ro` should have been the write-capable `cluster_composite_ops` (Dashboards' saved-objects backend appears to use composite `_bulk`/`_msearch` operations even for single-object writes), and `.kibana*` actions should be `["delete","index","manage","read"]` rather than plain `"crud"`. `implementations/wazuh/dashboard/rbac/roles.yml`/`README.md` updated to v2 with this fix, plus a previously-missed gap: the viewer had no `.kibana*` grant at all and likely could never have opened the dashboard either — v2 adds a read-only grant. **v2 has not been live-tested** — do not check this item off until it is retested per the updated `README.md` procedure and recorded in a new dated evidence file.)
